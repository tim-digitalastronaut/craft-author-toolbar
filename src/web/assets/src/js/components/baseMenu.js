...
export class BaseMenu extends HTMLElement {
	...
	connectedCallback() {
		if (!this.toggleElement || !this.menuElement || !this.anchorElement || !this.contentElement) {
			throw new Error(
				"Subclasses must set toggleElement, menuElement, contentElement, and anchorElement before calling super.connectedCallback()",
			);
		}
	...
	registerEventListeners() {
		...
		document.addEventListener("close-menus", () => {
			this.menuElement.classList.remove("open");
		});
		...
		if (this.toggleElement.tagName.toLowerCase() === "input") {
			this.toggleElement.addEventListener("focus", () => {
				this.menuElement.classList.toggle("open");
			});
		}
	}
